...
API_KEY = secret_manager.get_secret(
    secret_name="projects/48536241023/secrets/alpaca-api/versions/3"
)
SECRET_KEY = secret_manager.get_secret(
    secret_name="projects/48536241023/secrets/alpaca-secret-key/versions/3"
)


def fingerprint_is_up_to_date(
    fingerprint: str = None, symbol_type: str = "crypto"
) -> bool:
    if symbol_type == "crypto":
        now_timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S-00:00")
    else:
        now_timestamp = (datetime.now() - timedelta(minutes=15)).strftime(
            "%Y-%m-%d %H:%M:%S-00:00"
        )
...
def convert_columns_to_float64(data_frame: DataFrame, columns):
    for column in columns:
        data_frame[column] = data_frame[column].astype("float64")
    data_frame.reset_index(inplace=True)
    data_frame["timestamp"] = data_frame["timestamp"].astype(int) // 10**9
...
class AlpacaDataFunction:
    ...
    def get_history_batch(
        self, start_timestamp: str, end_timestamp: str, time_frame=TimeFrame.Minute
    ):
        raise NotImplementedError("This method must be implemented in a subclass")
    ...
    def convert_data_to_parquet(self, data: pd.DataFrame, latest_bar_data: str):
        data.to_parquet(
            path=f"/tmp/{latest_bar_data}_{self.symbol}.pq", compression="snappy"
        )
...
class AlpacaCryptoDataFunction(AlpacaDataFunction):
    ...
    def get_latest_data(
        ...
        data = self.api.get_crypto_bars(
            symbol=self.symbol, timeframe=time_frame.value
        ).df.iloc[[-1]]
    ...
    def set_date_range(self, start_timestamp: str):
        ...
        return {"start": start_timestamp, "end": end_timestamp, "freq": freq}
...
class AlpacaStockDataFunction(AlpacaDataFunction):
    ...
    def set_date_range(self, start_timestamp: str):
        if not self.update_history:
            ...
            end_timestamp = offset_time.strftime(self.time_format)
            return {"start": start_timestamp, "end": end_timestamp, "freq": "2m"}
        raise ValueError("The update_history must be False for stock data")


def main(
    symbol: str = "BTCUSD", bucket_name="crypto_data_collection", symbol_type="crypto"
):
    gcs_storage = CloudStorageUtils()

    fingerprint = gcs_storage.get_fingerprint_for_user(
        bucket_name=bucket_name, file_name=f"{symbol}/fingerprint.csv"
    )

    if fingerprint_is_up_to_date(fingerprint=fingerprint, symbol_type=symbol_type):

        logger.warning(
            f"Fingerprint is not up to date for {symbol}, historical data will be updated"
        )
        historical_data(
            symbol=symbol,
            start_timestamp=fingerprint,
            update_history=True,
...
def historical_data(
    ...
    if symbol_type == "stock":
        if update_history:
            ...
            date_range = [
                [alpaca.convert_fingerprint(start), end.strftime(alpaca.time_format)]
            ]
        else:
    ...
    for start, end in date_range:
        ...
        if not alpaca.check_data_empty(data=data):
            logger.warning(
                f"Saving historical data from : {start} to : {end} for {symbol} to cloud storage"
            )
            fingerprint, first_bar_data = alpaca.get_first_data_time(data=data)
            converted_data = alpaca.convert_columns_to_float64(
                data=data, columns=["open", "high", "low", "close", "volume"]
            )
            alpaca.convert_data_to_parquet(
                data=converted_data, latest_bar_data=first_bar_data
            )
            logger.warning(
                f"Saving {first_bar_data} data for {symbol} to cloud storage"
            )

            gcs_storage.save_data_to_cloud_storage(
                bucket_name=bucket_name,
